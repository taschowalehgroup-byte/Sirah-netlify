...
const UI = {
  async updateBadges() {
    try {
      ...
      // Insurance pending claims badge
      try {
        const insStats = await DB.fetch('/insurance/claims/stats');
        const insPending = (insStats.pending||0) + (insStats.submitted||0);
        const insBadge = $('badgeInsurance');
        if (insBadge) {
          insBadge.textContent = insPending;
          insBadge.style.display = insPending > 0 ? '' : 'none';
        }
      } catch(e) { /* badge stays hidden */ }

      // Lab orders pending badge
      try {
        const labOrders = await DB.tables.laborders.all().then(l=>l.filter(x=>x.status==='pending'));
        const labBadge = $('badgeLabPending');
        if (labBadge) {
          const cnt = Array.isArray(labOrders) ? labOrders.length : 0;
          labBadge.textContent = cnt;
          labBadge.style.display = cnt > 0 ? '' : 'none';
        }
      } catch(e) { /* badge stays hidden */ }
    } catch (e) {
