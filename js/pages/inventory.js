...
const InventoryPage = {
  ...
  renderTable(rows) {
    $('invBody').innerHTML = rows.length === 0
      ? `<tr><td colspan="8"><div class="empty-state"><div>📦</div><p>${typeof T!=="undefined"?T("No items found"):"No items found"}</p></div></td></tr>`
      : rows.map(i=>{
        ...
        return `
          <tr>
            ...
            <td>${low?'<span class="badge badge-emergency">${typeof T!=="undefined"?T("Low Stock"):"Low Stock"}</span>':'<span class="badge badge-confirmed">OK</span>'}</td>
